...
export interface BackendMemoryQuery {
  text: string; scopes: MemoryScope[]; subject?: string; limit: number; botAppId: string;
}
...
export class Mem0MemoryBackend extends TransportMemoryBackend {
  ...
  protected mapQuery(q: BackendMemoryQuery) { return { query: q.text, user_id: q.subject, limit: q.limit, filters: { scopes: q.scopes, botAppId: q.botAppId } }; }
}
export class HindsightMemoryBackend extends TransportMemoryBackend {
  ...
  protected mapQuery(q: BackendMemoryQuery) { return { query: q.text, bank_id: q.subject ? `user:${q.subject}` : undefined, top_k: q.limit, metadata_filter: { scopes: q.scopes, bot_app_id: q.botAppId } }; }
}
export class OpenVikingMemoryBackend extends TransportMemoryBackend {
  ...
  protected mapQuery(q: BackendMemoryQuery) { return { text: q.text, namespaces: q.scopes.map(scope => `${scope}/${q.subject ?? '*'}`), limit: q.limit, attributes: { bot_app_id: q.botAppId } }; }
}
